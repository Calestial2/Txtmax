...
const char *keywords_c[] = {"int",     "return", "if",   "else",
                            "while",   "for",    "void", "char",
                            "include", "define", NULL};
const char *keywords_python[] = {"def",   "return", "if",     "else",
                                 "while", "for",    "import", "in",
                                 "from",  "print",  "class",  NULL};
const char *keywords_java[] = {
    "public", "private", "protected", "class", "static", "void",   "new",
    "return", "if",      "else",      "while", "for",    "import", NULL};
const char *keywords_js[] = {"function", "var",    "let", "const",  "if",
                             "else",     "while",  "for", "return", "class",
                             "import",   "export", NULL};
const char *keywords_cpp[] = {
    "int",   "return",  "if",     "else",   "while",   "for",       "void",
    "class", "include", "define", "public", "private", "protected", NULL};
const char *keywords_rust[] = {"fn",   "let",   "mut", "struct", "impl",
                               "pub",  "mod",   "use", "return", "if",
                               "else", "while", "for", NULL};
const char *keywords_ruby[] = {"def",    "end",     "if",     "else",
                               "elsif",  "while",   "for",    "class",
                               "module", "require", "return", NULL};
const char *keywords_elixir[] = {"def",   "end",  "if",        "else",
                                 "do",    "case", "defmodule", "import",
                                 "super", "IO",   NULL};
const char *keywords_kotlin[] = {
    "fun", "val",   "var",    "return",    "if",     "else", "while",
    "for", "class", "object", "interface", "import", NULL};
const char *keywords_swift[] = {"func",  "let",    "var",    "if",
                                "else",  "while",  "for",    "return",
                                "class", "import", "struct", NULL};
const char *keywords_typescript[] = {
    "function", "let",   "const",     "if",     "else",   "while", "for",
    "return",   "class", "interface", "import", "export", NULL};
const char *keywords_dart[] = {"void",  "return", "if",     "else",
                               "while", "for",    "class",  "import",
                               "int",   "double", "String", NULL};
const char *keywords_go[] = {"func",    "package", "import", "var",    "const",
                             "if",      "else",    "for",    "return", "int",
                             "float64", "string",  NULL};
const char *keywords_haskell[] = {"import", "let",      "in",   "if",
                                  "then",   "else",     "data", "type",
                                  "class",  "instance", "do",   NULL};
const char *keywords_r[] = {"if",     "else",     "for",     "while",
                            "return", "function", "TRUE",    "FALSE",
                            "NULL",   "library",  "require", NULL};
const char *keywords_php[] = {
    "<?php",  "echo",  "if",     "else",    "for",     "while",   "function",
    "return", "class", "public", "private", "require", "include", NULL};
const char *keywords_lua[] = {"local",   "function", "if",     "else",
                              "for",     "while",    "return", "end",
                              "require", "print",    NULL};
const char *keywords_objc[] = {
    "@interface", "@implementation", "if",    "else",  "return", "int", "char",
    "void",       "NSString",        "NSLog", "while", "for",    "id",  NULL};
const char *keywords_groovy[] = {"def",    "if",      "else",   "for",
                                 "while",  "return",  "class",  "package",
                                 "public", "private", "static", NULL};
const char *keywords_erlang[] = {
    "module", "export", "import", "if",   "case",  "receive", "fun",
    "end",    "apply",  "spawn",  "node", "catch", NULL};
const char *keywords_bash[] = {"if",       "then",   "else",  "elif", "fi",
                               "for",      "while",  "until", "do",   "done",
                               "function", "return", NULL};
const char *keywords_julia[] = {
    "function", "end",   "if",      "else",  "elseif", "for", "while",
    "return",   "begin", "mutable", "const", "global", NULL};
const char *keywords_fsharp[] = {"let",   "in",     "if",   "then", "else",
                                 "match", "return", "type", "new",  "abstract",
                                 "class", "module", NULL};
const char *keywords_csharp[] = {
    "namespace", "class",  "public", "private", "protected", "static", "void",
    "int",       "return", "if",     "else",    "for",       "while",  NULL};
const char *keywords_scheme[] = {"define", "lambda",       "if",    "else",
                                 "cond",   "begin",        "quote", "set!",
                                 "let",    "define-macro", NULL};
const char *keywords_clojure[] = {"def",    "let",      "if",   "do",  "fn",
                                  "defn",   "quote",    "case", "map", "reduce",
                                  "filter", "defmulti", NULL};
const char *keywords_fortran[] = {
    "program",    "end",  "if",      "else", "do",        "return", "function",
    "subroutine", "type", "integer", "real", "character", NULL};
const char *keywords_cobol[] = {
    "IDENTIFICATION", "PROGRAM", "DIVISION", "DATA",   "SECTION",
    "PROCEDURE",      "IF",      "ELSE",     "END-IF", "DISPLAY",
    "MOVE",           "ACCEPT",  NULL};
const char *keywords_vhdl[] = {
    "architecture", "entity",   "if",       "else", "process",   "begin", "end",
    "signal",       "variable", "constant", "port", "procedure", NULL};
const char *keywords_lisp[] = {"defun",  "if",   "else", "cond",
                               "lambda", "let",  "setq", "setq",
                               "defvar", "loop", NULL};
const char *keywords_sql[] = {
    "SELECT", "FROM",    "WHERE", "INSERT",   "UPDATE", "DELETE",   "CREATE",
    "DROP",   "ALTER",   "JOIN",  "INNER",    "OUTER",  "GROUP BY", "ORDER BY",
    "LIMIT",  "HAVING",  "AND",   "OR",       "NOT",    "NULL",     "IS",
    "IN",     "BETWEEN", "LIKE",  "DISTINCT", "UNION",  NULL};
const char *keywords_matlab[] = {
    "if",         "else",   "elseif", "for",       "while", "end",     "return",
    "function",   "switch", "case",   "otherwise", "try",   "catch",   "global",
    "persistent", "import", "clear",  "clc",       "disp",  "fprintf", NULL};
const char *keywords_perl[] = {
    "sub", "if",      "else",    "elsif", "foreach", "while", "return", "my",
    "use", "require", "package", "print", "chomp",   "open",  "close",  "exit",
    "die", "qw",      "eval",    "BEGIN", "END",     NULL};
const char *keywords_scala[] = {"def",     "val",   "var",    "class",
                                "object",  "trait", "import", "package",
                                "extends", "with",  "return", "if",
                                "else",    "match", NULL};
const char *keywords_html[] = {"html", "head",  "body",  "title", "div",
                               "span", "p",     "a",     "img",   "script",
                               "link", "style", "table", "ul",    "ol",
                               "li",   "input", "form",  NULL};
const char *keywords_postgresql[] = {
    "SELECT", "INSERT",   "UPDATE",   "DELETE",   "CREATE", "DROP",
    "ALTER",  "GRANT",    "REVOKE",   "WHERE",    "FROM",   "JOIN",
    "HAVING", "GROUP BY", "ORDER BY", "DISTINCT", "LIMIT",  "RETURNING",
    "CAST",   "TABLE",    "SCHEMA",   NULL};
const char *keywords_nosql[] = {
    "db",    "find", "insert", "update",     "delete",     "aggregate", "match",
    "group", "sort", "limit",  "projection", "collection", NULL};
const char *keywords_actionscript[] = {
    "function", "var",     "const",      "if",     "else",    "while",
    "for",      "return",  "class",      "public", "private", "import",
    "package",  "extends", "implements", NULL};
const char *keywords_ada[] = {
    "procedure", "function", "if",   "else",    "begin",  "end", "loop",
    "return",    "declare",  "type", "private", "public", NULL};
const char *keywords_nim[] = {"proc", "var",    "const", "if",
                              "else", "for",    "while", "return",
                              "type", "import", NULL};
const char *keywords_crystal[] = {"def",    "return",  "if",     "else",
                                  "elsif",  "for",     "while",  "class",
                                  "module", "private", "public", NULL};
const char *keywords_d[] = {"void",   "int",    "if",     "else",
                            "for",    "while",  "return", "class",
                            "struct", "import", "alias",  NULL};
const char *keywords_purescript[] = {"let",     "if",     "else",     "case",
                                     "of",      "data",   "type",     "import",
                                     "foreign", "module", "instance", NULL};
const char *keywords_assembly[] = {"mov", "add",  "sub", "jmp",  "cmp", "je",
                                   "jne", "call", "ret", "push", "pop", NULL};
const char *keywords_prolog[] = {":-",     "if",   "else", "end",
                                 "assert", "fail", "true", "false",
                                 "query",  "fact", NULL};
const char *keywords_zig[] = {"const", "var", "fn",  "return", "if",    "else",
                              "while", "for", "try", "catch",  "defer", NULL};
const char *keywords_brainfuck[] = {">", "<", "+", "-", ".",
                                    ",", "[", "]", NULL};
const char *keywords_algol[] = {
    "begin",     "end",      "if",      "then", "else",    "for",   "while",
    "procedure", "function", "integer", "real", "boolean", "array", NULL};
const char *keywords_labview[] = {
    "while", "for",   "if",       "case",     "end",   "else", "true",
    "false", "break", "continue", "function", "subVI", NULL};
const char *keywords_mercury[] = {
    "module", "end_module", "import", "type",  "func",   "pred", "if",
    "then",   "else",       "true",   "false", "forall", NULL};
const char *keywords_dart_shell[] = {
    "#!/bin/bash", "echo",  "if",     "then",     "else",   "fi",
    "for",         "while", "return", "function", "export", NULL};
const char *keywords_ocaml[] = {"let",   "in",   "if",   "then",   "else",
                                "match", "with", "type", "module", "open",
                                "rec",   "and",  "fun",  NULL};
const char *keywords_txl[] = {"def",    "if",   "then",   "else",
                              "while",  "for",  "return", "class",
                              "module", "type", "eval",   NULL};
const char *keywords_xojo[] = {"dim",    "if",      "then",     "else",  "for",
                               "while",  "return",  "function", "class", "end",
                               "public", "private", NULL};
const char *keywords_forth[] = {"if",    "else", "then",  "begin",  "again",
                                "loop",  "do",   "while", "return", "create",
                                "colon", "exit", NULL};
const char *keywords_verilog[] = {
    "module", "endmodule", "input", "output", "wire", "assign", "always",
    "if",     "else",      "begin", "end",    "for",  NULL};
const char *keywords_awk[] = {"BEGIN", "END",   "if",   "else",   "while",
                              "for",   "print", "next", "return", "function",
                              "match", "split", NULL};
const char *keywords_turing[] = {
    "put",  "get",  "var",       "if",       "then",   "else", "elsif",
    "loop", "exit", "procedure", "function", "return", "end",  NULL};
const char *keywords_gdscript[] = {
    "func",    "if",  "elif",  "else", "for",        "while",  "match",
    "extends", "var", "const", "tool", "class_name", "return", NULL};
const char *keywords_fantom[] = {
    "class", "mixins", "const", "type", "static", "abstract", "new", "return",
    "if",    "else",   "while", "for",  "using",  "func",     NULL};
const char *keywords_zsh[] = {"if",       "then",   "else", "fi",     "for",
                              "while",    "do",     "done", "case",   "esac",
                              "function", "return", "echo", "export", NULL};
const char *keywords_autohotkey[] = {
    "if",    "else", "loop",   "return", "send",   "set",   "global",
    "local", "exit", "msgbox", "run",    "hotkey", "while", NULL};
const char *keywords_ceylon[] = {"module",    "import",   "class",  "object",
                                 "interface", "function", "value",  "variable",
                                 "if",        "else",     "return", "for",
                                 "while",     "switch",   "case",   NULL};
const char *keywords_chuck[] = {"if",    "else",  "for",  "while",  "until",
                                "spork", "class", "fun",  "return", "gain",
                                "dur",   "time",  "void", NULL};
const char *keywords_natural[] = {
    "DEFINE",  "IF",   "THEN", "ELSE",   "END-DEFINE", "READ", "WRITE",
    "DISPLAY", "MOVE", "ADD",  "DELETE", "RETURN",     NULL};
const char *keywords_pike[] = {
    "int",    "float",    "string", "if",      "else", "while", "for",
    "return", "function", "class",  "inherit", "void", NULL};
const char *keywords_oberon[] = {
    "MODULE", "IMPORT", "BEGIN", "END",       "IF",  "THEN",  "ELSE",
    "ELSIF",  "WHILE",  "DO",    "PROCEDURE", "VAR", "CONST", "TYPE",
    "RECORD", "ARRAY",  "OF",    "RETURN",    NULL};
const char *keywords_xml[] = {
    "<",   "</",        ">",       "/>",      "<?",      "?>",     "<!--",
    "-->", "<![CDATA[", "DOCTYPE", "ELEMENT", "ATTLIST", "ENTITY", NULL};
const char *keywords_yaml[] = {"-",    ":",     "?",    "|",     ">",
                               "true", "false", "null", "~",     ">",
                               "|",    "!!",    "key",  "value", NULL};
const char *keywords_json[] = {"{",   "}",     "[",     "]",    ":",
                               ",",   "true",  "false", "null", "\"",
                               "key", "value", NULL};
const char *keywords_vb[] = {
    "Dim",     "If",       "Then",      "Else",         "ElseIf",  "End If",
    "For",     "Each",     "Next",      "While",        "Do",      "Loop",
    "Sub",     "Function", "End Sub",   "End Function", "Return",  "Class",
    "Public",  "Private",  "Protected", "Static",       "New",     "Set",
    "Get",     "Property", "Module",    "Imports",      "Try",     "Catch",
    "Finally", "Throw",    "True",      "False",        "Nothing", "And",
    "Or",      "Not",      "Is",        "As",           NULL};
const char *keywords_html5[] = {
    "<!DOCTYPE", "<html>",     "<head>",   "<body>", "<title>",  "<div>",
    "<span>",    "<p>",        "<a>",      "<img>",  "<script>", "<link>",
    "<style>",   "<table>",    "<ul>",     "<ol>",   "<li>",     "<form>",
    "<input>",   "<textarea>", "<button>", "<nav>",  "<footer>", NULL};
const char *keywords_css[] = {
    "body",        "div",        "span",     "p",      "a",        "h1",
    "h2",          "h3",         "h4",       "h5",     "h6",       "width",
    "height",      "margin",     "padding",  "border", "color",    "font-size",
    "font-family", "display",    "position", "flex",   "grid",     "background",
    "box-shadow",  "text-align", "float",    "clear",  "overflow", "z-index",
    "transition",  "transform",  "media",    NULL};
const char *keywords_sass[] = {
    "@import",  "@mixin", "@include", "@extend",    "@function",   "@if",
    "@else",    "@for",   "@while",   "$",          "color",       "font-size",
    "padding",  "margin", "border",   "background", "box-shadow",  "display",
    "position", "flex",   "grid",     "text-align", "font-family", NULL};
const char *keywords_babel[] = {
    "import",      "export", "function", "class",       "let",     "const",
    "var",         "if",     "else",     "for",         "while",   "return",
    "async",       "await",  "promise",  "new",         "extends", "super",
    "constructor", "this",   "super",    "import.meta", NULL};
const char *keywords_sas[] = {
    "data",   "set",    "input",      "datalines", "proc", "run",
    "if",     "else",   "then",       "end",       "var",  "array",
    "retain", "format", "length",     "merge",     "sort", "by",
    "sum",    "mean",   "proc print", "proc freq", NULL};
const char *keywords_plsql[] = {
    "DECLARE",  "BEGIN",     "END",     "IF",     "ELSE",      "ELSIF",
    "FOR",      "LOOP",      "WHILE",   "CURSOR", "EXCEPTION", "SELECT",
    "INSERT",   "UPDATE",    "DELETE",  "COMMIT", "ROLLBACK",  "RETURN",
    "FUNCTION", "PROCEDURE", "TRIGGER", NULL};
const char *keywords_graphql[] = {"query",      "mutation",  "subscription",
                                  "type",       "interface", "union",
                                  "input",      "schema",    "enum",
                                  "directive",  "fragment",  "extends",
                                  "implements", "resolve",   "field",
                                  "scalar",     "ID",        "String",
                                  "Int",        "Float",     "Boolean",
                                  "List",       NULL};
const char *keywords_sqlite[] = {
    "SELECT",   "INSERT",   "UPDATE",   "DELETE", "FROM",   "WHERE",
    "ORDER BY", "GROUP BY", "HAVING",   "JOIN",   "INNER",  "LEFT",
    "RIGHT",    "OUTER",    "DISTINCT", "LIMIT",  "OFFSET", "CREATE",
    "DROP",     "ALTER",    "TABLE",    "INDEX",  "VIEW",   "TRANSACTION",
...
void syntax_highlight(const char *line, const char *extension) {
  ...
  } else if (strcmp(extension, ".psql") == 0 ||
             strcmp(extension, ".pgsql") == 0) {
...
void git_commit(const char *filename, const char *commit_message,
                const char *branch, const char *version) {
...
void view_file(const char *filename) {
  ...
  if (!ext)
    ext = ""; // Default to empty string if no extension
...
void edit_file(const char *filename, int line_to_edit) {
  ...
  freopen(filename, "w",
          file); // Reopen file in write mode to overwrite content
...
void deploy() { system("gcc deploy.c -o deploy && ,/deploy"); }
...
void format_code() {
  ...
  if (strcmp(choice, "clang-format") != 0 && strcmp(choice, "black") != 0) {
    fprintf(
        stderr,
        "Invalid choice. Please choose either 'clang-format' or 'black'.\n");
    return;
  }
...
void quick_run() {
  ...
  printf("Enter the compiler or interpreter (gcc, g++, clang, python, node, "
         "nasm, tcl, bash, ruby, perl, rustc, go): ");
  ...
  if (strcmp(compiler, "gcc") == 0 || strcmp(compiler, "clang") == 0) {
    if (strcmp(ext, ".c") == 0) {
      snprintf(command, sizeof(command), "%s %s %s -o output && ./output",
               compiler, filename,
               strcmp(libraries, "n") == 0 ? "" : libraries);
    } else {
    ...
    if (strcmp(ext, ".cpp") == 0 || strcmp(ext, ".cxx") == 0 ||
        strcmp(ext, ".cc") == 0) {
      snprintf(command, sizeof(command), "g++ %s %s -o output && ./output",
               filename, strcmp(libraries, "n") == 0 ? "" : libraries);
    } else {
      printf(
          "Error: g++ is only compatible with C++ files (.cpp, .cxx, .cc).\n");
      return;
    }
    ...
    if (strcmp(ext, ".asm") == 0) {
      snprintf(command, sizeof(command),
               "nasm -f elf64 %s -o output.o && gcc %s output.o -o output && "
               "./output",
               filename, strcmp(libraries, "n") == 0 ? "" : libraries);
    } else {
    ...
    if (strcmp(ext, ".rs") == 0) {
      snprintf(command, sizeof(command), "rustc %s %s -o output && ./output",
               filename, strcmp(libraries, "n") == 0 ? "" : libraries);
    } else {
...
void packages() {
  ...
  printf(
      "Enter package manager (pip/npm/gem/go/yarn/composer/cargo/nuget/apt): ");
...
void api() {
  ...
  char command[MAX_URL_LENGTH +
               10]; // To accommodate "curl " and null terminator
  ...
  if (snprintf(command, sizeof(command), "curl \"%s\"", url) >=
      sizeof(command)) {
...
void sqlite() {
  ...
  snprintf(system_command, sizeof(system_command), "sqlite3 %s < %s", filename,
           filename);
...
void searchInFile(FILE *file, const char *fileName,
                  const char *searchPattern); // Pass fileName
...
void searchInFile(FILE *file, const char *fileName,
                  const char *searchPattern) { // Receive fileName
  char buffer[MAX_LINE_LENGTH];
  ...
  while (fgets(buffer, sizeof(buffer), file) != NULL) {
    ...
    if (regexec(&regex, buffer, 0, NULL, 0) == 0) {
      ...
      if (scanf(" %[^\n]s", replacement) !=
          1) { // Read replacement text including spaces
...
void tmux_integration() {
  ...
  snprintf(command, sizeof(command), "tmux new-session -d -s \"%s\"",
           session_name);
  ...
  while (1) {
    ...
    snprintf(command, sizeof(command), "tmux send-keys -t \"%s\" \"%s\" Enter",
...
void tarball() {
  ...
  snprintf(command, sizeof(command), "tar -czvf %s.tar.gz %s", project_name,
           filename);
  printf("Executing: %s\n", command);
...
void openai() {
  ...
  fprintf(file,
          ...
          "    sys.exit(1)\n\n"
          "# Combine all command line arguments (excluding the script name) "
...
void markdown() {
  ...
  while (1) {
    ...
    if (!fgets(line, MY_MAX_INPUT, stdin))
      break;
...
void manualAI() {
  ...
  printf("\nHow to Use:\n");
  printf("1. Type a query related to Txtmax, and AI will respond with relevant "
         "commands.\n");
  ...
  printf("3. If the AI does not understand your query, try rewording it to "
         "match Txtmax commands.\n");
...
void ai() {
  ...
  if (strstr(input, "run my code") || strstr(input, "how to run")) {
    ...
    printf("- Step 1: \"Run Command\"\n  Type \"run\" in Txtmax, and it will "
           "ask for the filename including its extension.\n");
    printf("- Step 2: \"Choosing Compiler\"\n  Choose a compiler or "
           "interpreter such as GCC, Clang, Python, Node, Ruby, etc.\n");
    printf("- Step 3: \"Run Your Code\"\n  Txtmax will seamlessly run your "
           "code.\n");
  } else if (strstr(input, "examples of hello world") ||
             strstr(input, "hello world example")) {
    printf("\nType 'examples' command in Txtmax to get Hello World examples in "
           "various languages.\n");
  } else if (strstr(input, "create a file") || strstr(input, "how to create")) {
    printf("\nTo create a file in Txtmax, type:\n  \"create <filename>\"\nThen "
           "write contents and save with :wq.\n");
    ...
    printf("\nTo search for a file, type:\n  \"search <filename>\"\nIt will "
           "locate files matching the given name.\n");
  } else if (strstr(input, "list all files") ||
             strstr(input, "view files in directory")) {
    printf(
  ...
  } else if (strstr(input, "debug c files") ||
             strstr(input, "check c code errors")) {
    printf("\nTo debug C files, type \"debug\" and provide the project name "
           "and filename.\n");
  } else if (strstr(input, "test apis") ||
             strstr(input, "fetch restful api data")) {
    printf("\nTo test RESTful APIs, use:\n  \"api\" or \"axios\" command.\n");
  } else if (strstr(input, "convert file to tarball") ||
             strstr(input, "make tar file")) {
    printf("\nTo create a tarball file, type:\n  \"tarball <filename>\"\n");
  } else if (strstr(input, "install package") ||
             strstr(input, "package manager")) {
    printf("\nTo install a package, use:\n  \"packages\" command and choose a "
           "package manager.\n");
  } else if (strstr(input, "latex files") || strstr(input, "edit latex")) {
    printf(
        "\nTo create and edit LaTeX files, use:\n  \"latex <filename>.tex\"\n");
  } else {
    printf("\nI'm sorry, I don't understand your query. Try asking something "
           "related to Txtmax commands!\n");
  }
}
...
void copy_files_to_folder() {
  ...
  printf(
      "Enter filenames with extension (separate with spaces, max 10 files): ");
  ...
  for (int i = 0; i < num_files; i++) {
    ...
    copy_file(
        files[i],
        destination); // Call the copy_file function with the right arguments
  }
}

void generateGradleScript() {
  char pluginName[100], dependencyName[100], dirName[100], taskName[100],
      printMessage[100];
  FILE *gradleFile;
...
void create_dotnet_project() {
  ...
  if (strcmp(component, "dotnet sdk") != 0 &&
      strcmp(component, "asp.dotnet") != 0) {
    printf("Invalid component. Exiting...\n");
    return;
  }

  // Step 2: Ask for the directory path (optional, default is current directory)
  printf("Enter the directory to save the file (or press enter to save in the "
         "current directory): ");
...
void man_txtmax() {
  ...
  printf("                      DESCRIPTION\n");
  printf("       Txtmax is a lightweight, fast, and fully open-source text "
         "editor designed for Linux and Unix-like\n");
  printf("       systems. It provides an easy-to-use command-line interface "
         "for text file creation, editing,\n");
  printf("       viewing, and management. Txtmax supports syntax highlighting, "
         "integrated code execution, file\n");
  printf("       versioning via Git, and many advanced features for "
         "developers.\n\n");
  ...
  printf("           View the contents of a specified text file with syntax "
         "highlighting for code readability.\n\n");
  ...
  printf("           Retrieve detailed information about a file, including its "
         "name, extension, creation time,\n");
  printf("           modification time, version control, commit message, "
         "branch, and version.\n\n");

  printf("       examples\n");
  printf("           Show example code snippets for popular programming "
         "languages like C, Python, Java, JavaScript,\n");
  printf("           C++, Rust, Ruby, Kotlin, and more.\n\n");

  printf("       sql\n");
  printf("           Show SQL examples and query syntax for relational and "
         "NoSQL databases like MySQL, PostgreSQL, and MongoDB.\n\n");

  printf("       run\n");
  printf("           Run code written in the text file. Txtmax supports "
         "running code using compilers and interpreters\n");
  printf("           like GCC, Clang, Python, Node.js, and more.\n\n");

  printf("       packages\n");
  printf("           Install Python (pip) or Node.js (npm) or Gem (Ruby) or Go "
         "(Golang) and many other packages from the terminal. You can choose "
         "the package manager\n");
  ...
  printf("       advance\n");
  printf("           Open File and jump into an content and search for "
         "specific text/code in it.\n\n");
  ...
  printf("       benchmark\n");
  printf("             Shows Execution time and CPU Usage and Additional "
         "things like System CPU Time and Total CPU Time and Max Memory usage "
         "and Page Faults and Context Switches.\n\n");
  ...
  printf(
      "       - Run Code Quickly with Integrated Compilers and Interpreters\n");
  printf(
      "       - Git Integration for Version Control and Commit Management\n");
  printf("       - Support for Over 80 Programming Languages\n");
  printf("       - Installation of Python and Node.js Packages and many "
         "more!\n\n");
  ...
  printf("       Alternatively, use the Makefile to install the latest version "
         "with:\n");
  printf("           make ./txtmax\n\n");
  ...
  printf("       C, Python, Java, JavaScript, C++, Rust, Ruby, Kotlin, Swift, "
         "TypeScript, Dart, Go, Haskell,\n");
  printf("       R, PHP, Lua, Objective-C, Groovy, Erlang, Bash, Julia, F#, "
         "C#, Scheme, Clojure, Fortran, Cobol,\n");
  printf("       VHDL, Lisp, SQL, Matlab, Perl, Scala, PostgreSQL, NoSQL, "
         "ActionScript, Ada, Nim, Crystal,\n");
  printf("       D, Assembly, Purescript, Prolog, Zig, Brainfu*k, HTML and "
         "many More\n\n");

  printf("                    LICENSE\n");
  printf("       Txtmax is licensed under the GNU General Public License v3.0. "
...
void file_info(const char *filename) {
  ...
  const char *ext = strrchr(filename, '.');
  if (!ext)
    ext = "None"; // Default to "None" if no extension
...
void help() {
  ...
  printf("  movef                   Move Single or Multiple Files to an "
         "Folder.\n");
  printf("  files                   List all files in the current directory\n");
  printf(
      "  search <filename>       Search for files with the specified name\n");
  printf("  view <filename>         View file contents with syntax "
         "highlighting\n");
  printf("  edit <filename> <line>  Edit a specific line in the file\n");
  printf("  delete <filename>       Delete a file\n");
  printf("  info <filename>         Show file info (name, extension, creation "
         "time, modification time)\n");
  printf("  run                     Excute Your Code without exiting\n");
  printf("  packages                Install Packages\n");
  printf("  axios                   Fetch Data with Axios\n");
  printf(
      "  api                     Test Restful APis directly in the editor\n");
  printf("  advance                 Open Files and Jump to Specfic line of it "
         "and Search for an Specfic Text/Code\n");
  printf("  examples                Show Hello World examples in various "
         "languages\n");
  ...
  printf(
      "  manai                   Comprehensive Manual of How to use Txtmax\n");
  printf(
      "  copy                    Copy Single or Multiple files to an Folder\n");
  printf("  make                    Build and Test a Project with Make\n");
  printf(
      "  gradle                  Build and Test an Java Project with Gradle\n");
  printf("  dotnet                  Supports .NET Framework\n");
  printf("  recycle                 Recover your all Deleted files\n");
  printf("  localhost               Start your Flask and Python App on "
         "localhost\n");
  ...
  printf("  benchmark               Shows Execution time and CPU Usage and "
         ...
         "Memory usage and Page Faults and Context Switches.\n");
...
int main() {
  ...
  printf("Welcome to Txtmax, a Powerful Text Editor and IDE for Linux and "
         "Unix-like Systems\n\n");
  printf("* Documentation: "
         "https://github.com/Calestial2/Txtmax/blob/main/README.md\n");
